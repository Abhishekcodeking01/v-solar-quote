...
<head>
    ...
    <title>${plantKw} KW Rooftop Solar Proposal</title>
    <!-- Tailwind CSS -->
...
function buildSummaryQuotationHtml(totals, systemType) {
  ...
  const rows = totals.items.map((it, idx) => {
    // Hide individual line prices - show only description and qty
    return `
      <tr class="odd:bg-white/50 even:bg-gray-50/50">
        <td class="p-3 border text-center">${idx + 1}</td>
        <td class="p-3 border">${it.item}</td>
        <td class="p-3 border">${escapeHtml(it.desc || '-')}</td>
        <td class="p-3 border text-center">${it.qty}</td>
        <td class="p-3 border text-center">${it.unit}</td>
      </tr>`;
  }).join('');
...
<head>
    ...
    <style>
      ...
      .page-container { background: white; max-width: 210mm; margin: 0 auto; padding: 40px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); }
    </style>
    <script>
        function printMode(mode) {
            document.body.className = 'font-sans text-gray-800'; // Reset
...
<body class="text-gray-800">
    ...
    <div class="page-container">
        <!-- Header -->
        <div class="flex justify-between items-start mb-8 border-b-2 border-brand-blue pb-4">
            <div class="w-32">
                <img src="https://github.com/Abhishekcodeking01/v-solar-quote/blob/9ae39ab1ba9eb2eedc38678b5d67f65a93283d84/Uplodes/v%20sustain%20logo.png?raw=true" alt="Logo" class="w-full">
            </div>
            <div class="text-right">
                <h1 class="text-2xl font-bold text-brand-blue">Summary Quotation</h1>
                <p class="text-sm text-gray-500">Date: ${proposalDate}</p>
                <p class="text-sm text-gray-500">To: ${escapeHtml(customerName)}</p>
                <p class="text-sm text-gray-500">${plantKw} KW Rooftop Solar</p>
            </div>
        </div>
        ...
        <div class="flex justify-end mt-4">
            <div class="bg-blue-50 p-4 rounded-lg border border-blue-100 text-right w-1/2">
                <p class="text-lg font-bold text-brand-blue">Grand Total: ${fmt(totals.grandTotal)}</p>
        ...
        <div class="mt-12 pt-4 border-t border-gray-200 text-center text-xs text-gray-500">
            <p>V-Sustain Solar Solutions | Authorized Luminous Partner</p>
        </div>
    </div>
</body>
</html>`;
